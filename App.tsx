  ...
  <NavigationContainer>
    {!erLoggetinn ? (
      ...
      <Tab.Navigator screenOptions={({ route }) => ({
          tabBarIcon: ({ color }) => {
          let ikon;
          if (route.name === 'History') {ikon = historyIcon;} 
            else if (route.name === 'Home') {ikon = homeIcon; } 
            else if (route.name === 'Training') {ikon = playIcon; } 
            else if (route.name === 'Settings') {ikon = settingsIcon;}
            return (<Image source={ikon} style={{ marginTop: 15, width: 40, height:40, tintColor: color }} />)},
